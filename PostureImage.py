...
[85, 255, 0],
[0, 255, 0], \
[0, 255, 85], [0, 255, 170], [0, 255, 255], [0, 170, 255], [0, 85, 255], [0,
0, 255],
[85, 0, 255], \
...
multiplier = [x * model_params['boxsize'] / oriImg.shape[0] for x in
params['scale_search']]
...
util.padRightDownCorner(imageToTest, model_params['stride'],
...
np.transpose(np.float32(imageToTest_padded[:,:,:,np.newaxis]), (3,0,1,2)) #
...
heatmap = cv2.resize(heatmap, (oriImg.shape[1],

oriImg.shape[0]), interpolation=cv2.INTER_CUBIC)
...
(map >= map_left, map >= map_right, map >= map_up, map >=
map_down, map > params['thre1']))
...
calcDistance(all_peaks[3][0][0:2],all_peaks[4][0][0:2]) #distance between
right arm-joint and right palm.
...
(armdist - 100) ): #this value 100 is arbitary. this shall be replaced with a
calculation which can adjust to different sizes of people.
...
distance = calcDistance( all_peaks[6][0][0:2]

,all_peaks[7][0][0:2])

armdist = calcDistance(all_peaks[6][0][0:2],
...
f = 1
try:
if(all_peaks[10][0][0:2] and all_peaks[13][0][0:2]): # if both legs
...
if (leftdegrees > 60 and rightdegrees > 60): # 60 degrees is trail
and error value here. We can tweak this accordingly and results will vary.
...
try:
if (f):
a = all_peaks[10][0][0:2] # if only one leg (right leg)

is detected

else:
a = all_peaks[13][0][0:2] # if only one leg (left leg)
...
def showimage(img): #sometimes opencv will oversize the image when using
using `cv2.imshow()`. This function solves that issue.
...
canvas, position= process('C:/Users/lenovo/Desktop/DSP
Project/Images/2.jpeg', params, model_params)
showimage(canvas)
if (position == 1):
print("Hunchback")
elif (position == -1):
